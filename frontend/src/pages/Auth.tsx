...
export default function Auth() {
  ...
  // Types de comptes avec routes spécifiques
  const accountTypes = [
    {
      value: 'customer',
      label: 'Client',
  ...
  const handleAccountTypeSelection = (route: string) => {
    navigate(route);
  };
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {accountTypes.map((type) => {
                      ...
                      return (
                        <div
                          key={type.value}
                          className="border-2 border-gray-200 rounded-lg p-4 cursor-pointer hover:border-indigo-300 hover:shadow-md transition-all"
                          onClick={() => handleAccountTypeSelection(type.route)}
                        >
