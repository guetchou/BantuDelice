  ...
  const trackingEvents = useMemo(() => {
    ...
    const events: TrackingEvent[] = [
      {
        ...
        timestamp: trackingInfo.createdAt,
        icon: Package
      }
    ];

    // Ajouter des événements simulés basés sur le statut
    if (trackingInfo.status !== 'pending') {
      events.push({
        ...
        location: trackingInfo.sender.address,
        timestamp: new Date(new Date(trackingInfo.createdAt).getTime() + 2 * 60 * 60 * 1000).toISOString(),
        icon: CheckCircle
      });
    }
    ...
    if (trackingInfo.status === 'in_transit' || trackingInfo.status === 'delivered') {
      events.push({
        ...
        timestamp: new Date(new Date(trackingInfo.createdAt).getTime() + 4 * 60 * 60 * 1000).toISOString(),
        icon: Truck
      });
    }
    ...
    if (trackingInfo.status === 'delivered') {
      events.push({
        ...
        timestamp: trackingInfo.estimatedDelivery || new Date().toISOString(),
        icon: CheckCircle
      });
    }
