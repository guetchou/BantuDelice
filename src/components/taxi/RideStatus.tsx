...
Let me help you enhance the taxi booking experience with a more elegant, futuristic design and improved functionality. I'll update the RideStatus component to make it more dynamic and visually appealing.

<lov-code>
First, let's update the RideStatus component to fix the TypeScript errors and improve the design:

<lov-write file_path="src/components/taxi/RideStatus.tsx">
import React, { useEffect, useState } from 'react';
import { useParams } from 'react-router-dom';
import { supabase } from "@/integrations/supabase/client";
import { Card } from "@/components/ui/card";
import { Badge } from "@/components/ui/badge";
import DeliveryMap from "@/components/DeliveryMap";
import { Clock, MapPin, Car, CreditCard, Star, Phone, MessageSquare } from "lucide-react";
...
const RideStatus = () => {
  ...
  const getStatusBadge = (status: string) => {
    const variants: Record<string, { variant: "default" | "secondary" | "destructive", label: string, icon: React.ReactNode }> = {
      'pending': { variant: "secondary", label: "En attente d'un chauffeur", icon: <Clock className="h-4 w-4" /> },
      'accepted': { variant: "default", label: "Chauffeur en route", icon: <Car className="h-4 w-4" /> },
      'in_progress': { variant: "default", label: "En course", icon: <MapPin className="h-4 w-4" /> },
      'completed': { variant: "default", label: "Terminée", icon: <Star className="h-4 w-4" /> },
      'cancelled': { variant: "destructive", label: "Annulée", icon: <Clock className="h-4 w-4" /> }
    };
