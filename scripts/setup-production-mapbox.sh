...
# Script de configuration automatique Mapbox Production
# Usage: ./scripts/setup-production-mapbox.sh

echo "🚀 Configuration de Mapbox Production pour Buntudelice"
echo "=================================================="
...
# Demander la clé API Mapbox de production
echo -e "${BLUE}🔑 Configuration de votre clé API Mapbox de production${NC}"
read -p "Entrez votre clé API Mapbox de production (pk.xxx...): " MAPBOX_TOKEN
...
if [[ ! $MAPBOX_TOKEN =~ ^pk\..* ]]; then
    echo -e "${RED}❌ Format de clé API invalide. Doit commencer par 'pk.'${NC}"
    exit 1
fi

echo -e "${GREEN}✅ Format de clé API valide${NC}"
...
echo "Configuration de MAPBOX_ACCESS_TOKEN..."
...
echo "• Clé API Mapbox: ${MAPBOX_TOKEN:0:10}..."
...
echo ""
echo -e "${GREEN}✅ Votre API Mapbox de production est maintenant configurée !${NC}" 